...
function setupLizard(size, legs, tail) {
  ...
  setInterval(function () {
    ...
    // Update reptile movement
    critter.follow(Input.mouse.x, Input.mouse.y);

    // Draw box
    drawTarget();

    // Check if reptile touches box
    checkCollision();
  }, 33);
}
...
var legNum = Math.floor(1 + Math.random() * 10);
setupLizard(
  4 / Math.sqrt(legNum),
  legNum,
...
var target = {
  ...
  size: 120, // you can change this
};

function drawTarget() {
  ctx.drawImage(targetImg, target.x, target.y, target.size, target.size);
}
...
function checkCollision() {
  var r = 10; // creature radius
  if (
    critter.x + r > target.x &&
    critter.x - r < target.x + target.size &&
    critter.y + r > target.y &&
    critter.y - r < target.y + target.size
  ) {
    respawnTarget();
  }
}